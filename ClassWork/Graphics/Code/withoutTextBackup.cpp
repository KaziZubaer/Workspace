...
Color getColorAt(int idx,PT p) {
	if(idx==1) {
		int i=(int)(p.x-(-CHECKER_BOARD_SIZE))/CHECKER_BOX_SIZE;
		int j=(int)(p.y-(-CHECKER_BOARD_SIZE))/CHECKER_BOX_SIZE;
		Color ret;
		if((i&1)^(j&1)) ret.x=ret.y=ret.z=1;
		else ret.x=ret.y=ret.z=0;
		return ret;
	}
